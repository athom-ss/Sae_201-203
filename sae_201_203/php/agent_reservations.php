...
<?php 
try {
    $sql = "SELECT r.*, m.image AS image_materiel, m.type_materiel, i.nom AS nom_reservant, i.prenom AS prenom_reservant
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "<tr>
                ...
                <td>" . htmlspecialchars($row['statut']) . "</td>";
        // Affichage du nom et prénom depuis la table inscription si dispo, sinon depuis la réservation
        echo "<td>" . htmlspecialchars($row['nom_reservant'] ?? $row['nom_reservation']) . "</td>";
        echo "<td>" . htmlspecialchars($row['prenom_reservant'] ?? $row['prenom_reservation']) . "</td>";
        echo "<td>" . htmlspecialchars($row['num_carte_reservation']) . "</td>
              </tr>";
    }
    echo "</tbody></table>";
} catch (PDOException $e) {
